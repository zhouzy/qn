...
!function(window){
    ...
    $.extend(Pager.prototype,{
        ...
        _createPager:function($parent){
            ...
            $parent.addClass('pag-box').append('<span class="pag-items"></span>', $pager);
        },
        ...
        _updatePagerBtn:function(){
            ...
            var curLastNum = 0;
            if(total !== 0) {
                curFirstNum = (page - 1) * pageSize + 1;
                curLastNum = page * pageSize >= total ? total : page * pageSize;
            }

            this.$element.find('span.pag-items').text('当前第 ' + curFirstNum + '-' + curLastNum + ' 条 共计 ' + total + ' 条');
        },
        ...
        update:function(page,pageSize,total){
            var op = this.options;
            // 避免total为0时出错
            op.total = total != null ? total : op.total;
...
!function(window){
    ...
    TimeInput.prototype._init = function(){
        ...
        if(this.options.HMS && !/hh|mm|ss/.test(this.options.format)){
    ...
    TimeSelectView.prototype = {
        ...
        _yearSelect : function(){
            var that = this;
            var date = this.date;
            var $yearSelect = $('<div class="year_Selecter col-sm-6"></div>');
            var $minusBtn   = $('<span class="input-group-btn"><button class="btn btn-xs minus" type="button"> - </button></span>');
            var $plusBtn    = $('<span class="input-group-btn"><button class="btn btn-xs" type="button">+</button></span>');
        ...
        _monthSelect:function(){
            ...
            var $selectWrap = $('<div class="col-sm-6"></div>');
            ...
            return $selectWrap.append($inputGroup.append($select,'<span class="form-control-static">月</span>'));
        },
                ...
                for(var j=0; j<7;j++){
                    var $td = $("<td></td>").on("click",function(){
                        ...
                        if(day) {
                            ...
                            if(day != that.date.dd) { // 如果点击的不是当前选中天数
                                that.date.dd = +day;
...
/*                        if(day && day!=that.date.dd){
         ...
         */
        _setPosition:function(){
            var timeInputHeight = this.$parent[0].clientHeight;
            var left = this.$parent.offset().left + this.$parent.find('label').outerWidth();
            var top = this.$parent.offset().top + timeInputHeight;
            var scrollHeight = document.documentElement.clientHeight;
            //var scrollHeight = document.body.scrollHeight;
            if(top + 290 > scrollHeight) {
                this.$timeBox.addClass('top');
                this.$timeBox.css({
                    top: 'auto',
                    bottom: scrollHeight - (top - timeInputHeight),
                    left: left
                });
            } else {
                this.$timeBox.css({
        ...
        getDate:function(){
            var date = this.date;
            return date.dd == null ? '' : new Date(date.yyyy,date.MM,date.dd,date.HH,date.mm,date.ss);
        },
