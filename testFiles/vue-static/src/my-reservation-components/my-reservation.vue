...
<template>
    <div class="reservation-list-grid">
        <page-header topTitle="我的预约"></page-header>
        ...
        <transition name="slide">
            <div class="bottom-collapse-toolbar show" v-if="hasSelected">
                ...
                <a class="btn btn-sm btn-raised btn-primary" data-toggle="modal" data-target="#editReserve" v-if="this.$store.state.options['/appointment/edit/my'] !== undefined">{{this.$store.state.options['/appointment/edit/my']}}</a>
                <button class="btn btn-sm btn-raised btn-danger"  @click="deleteItem" v-if="this.$store.state.options['/appointment/delete/my'] !== undefined">{{this.$store.state.options['/appointment/delete/my']}}</button>
...
<script type="text/ecmascript-6">
	...
	export default{
		...
		data : function () {
			...
			return {
                isCompleteBtn: true, //预约未完成，则选中时不显示未完成按钮
				searchFormData : {
					userName  : "", //名单姓名
					status    : "", //名单
					telPhone  : "", //名单电话
		...
		methods: {
			...
			initGrid : function () {
				...
				this.grid = $("#grid").datagrid({
					url       : config.ORIGIN_URL + '/appointment/queryMyAppointment',
			...
			updateItem : function(item,cb){
				...
				$.post(config.ORIGIN_URL + '/appointment/edit/my',req,function(resp){
					if(resp.success){
			...
			//删除预约
			deleteItem : function(){
				$.post(config.ORIGIN_URL + '/appointment/delete/my',{id:this.selected.appointmentId},function(resp){
					if(resp.success){
