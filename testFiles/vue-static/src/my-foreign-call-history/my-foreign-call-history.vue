<template>
    <div class="my-foreign-call-history">
        <page-header top-title="我的外呼历史">
        </page-header>
        <div>
            <div class="my-foreign-call-history-box">
                <query-panel @clear="clear" @query="query">
                ...
                <search-result :total="pagertotals" :searchResult="searchResult" @page-update="updatePaper" :isResetPage="isResetPage" @export-history="exportHistory"></search-result>
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods   : {
            ...
            exportHistory: function () {
                let self = this;
                // isMine("0"：外呼历史,"1"：我的外呼历史)
                let obj = JSON.parse(JSON.stringify(self.cacheQueryFields));
                obj.isMine = '1'; // isMine("0"：外呼历史,"1"：我的外呼历史)
                this.axios({url:'/teleCommunicate/exportHistory/my',params: obj}).then(res => {
                    if(!res.data.success){
                        return notice.danger(res.data.msg);
                    }
...
<style lang="less" scoped>
    .my-foreign-call-history{
    }
    .my-foreign-call-history-box{
        height: 100%;
        overflow: auto;
    }
</style>