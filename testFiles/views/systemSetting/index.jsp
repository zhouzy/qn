...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/static/css/systemSetting.css">
</head>
<body>
<div id="left-part" class="page__left-sidebar">
    <div class="qn-container">
        <header class="qn-container__header">系统设置</header>
        <div class="qn-container__body">
                    ...
                    <ul class="qn-nav-bar__list" id="group1Menu">
                        <c:forEach items="${permissionList}" var="item">
                            <c:if test="${item.groupId == '1'}">
                                <li id="li${item.id}" class="qn-nav-bar__list-item" data-href="${item.url}" data-isajax="${item.isAjax}"><a>${item.name}</a></li>
                            </c:if>
                        </c:forEach>
                    </ul>
                </div>
            ...
            <c:if test="${hasGroup2}">
                <div class="qn-nav-bar">
                    <div class="qn-nav-bar__header">支持渠道</div>
            ...
            <c:if test="${hasGroup3}">
                <div class="qn-nav-bar">
                    <div class="qn-nav-bar__header">账号</div>
