    ...
    <div class="panel" style="background:#fff;padding:20px;margin-bottom:40px;">
        <div class="kf5-section">
            <header class="kf5-list-header">
                ...
                <a role="button" class="btn-sm green fr" href="<%=request.getContextPath()%>/queryUserCondition/goEdit" target="_self">添加新的筛选器</a>
            </header>
            <ul id="field_list" class="kf5-list1 move">
                <c:forEach items="${activeList}" var="item">
                    <li>
                        <div id="${item.conditionId}" class="col-md-3 ellipsis">${item.title}</div>
                        ...
                        <div>
                            ...
                            <c:if test="${item.isDefault =='0'}">
                                <a class="btn-sm fr" href="<%=request.getContextPath()%>/queryUserCondition/goEdit?conditionId=${item.conditionId}"
                                   target="_self">编辑</a>
            ...
            <ul class="kf5-list1 undertint">
                <c:forEach items="${deactiveList}" var="item">
                    <li>
                        ...
                        <div>
                            <a class="btn-sm fr" href="<%=request.getContextPath()%>/queryUserCondition/goEdit?conditionId=${item.conditionId}" target="_self">编辑</a>
...
<script type="text/javascript">
    ...
    function model_change(type,conditionId, active){
        var url = "<%=request.getContextPath()%>/queryUserCondition/" + type;
    ...
    setDragSort(sortCtner,sortItems,handle,function(){
        ...
        $.post("<%=request.getContextPath()%>/queryUserCondition/sortCondition",{type:"sort",conditionIds:ids},function(data){
            if(data.success){
