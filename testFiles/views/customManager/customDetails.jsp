...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/static/css/userDetails.css">
                            ...
                            <div class="btn-group pull-right" style="margin:-7px 0 0 0;">
                                ...
                                <div class="btn-group">
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        ...
                                        <li>
                                            <a id="merge" data-toggle="modal" onclick="goMerge();" data-target="#userMerge">合并客户</a>
                                        </li>
                                        <li>
                                            <a data-toggle="modal" onclick="goMake();" data-target="#bckTrckingModal">补录沟通小结</a>
                                        </li>
                                        <li>
                                            <a id="del" class="red" onclick="goDel()">删除客户</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                                ...
                                <div class="col-lg-12 col-md-12">
                                    <div class="user-info">
                                        ...
                                        <div class="drop-btn-default">
                                            <div id="isAssociatedWeChat" class="dropdown">
                                                <a href="javascript:void(0);">是否已关联微信：<span class="status green" id="associatedWeChatDesc">否</span></a>
                                            </div>
                                        </div>
                                        <%--状态--%>
                                        <div class="drop-btn-default">
...
<script>
    $(function () {
        // 创建工单权限控制
        var permissionArr = JSON.parse(localStorage.getItem('optionControl'));
        $('.createWorkOrder').hide();
        permissionArr.forEach(function (item) {
            if (item.url === 'newwork/index') {
        ...
        // 初始化关联微信状态
        $('#associatedWeChatDesc').text((USER_G.weixin != null && USER_G.weixin !== '') ? '是' : '否');
        // 客户信息初始化
        userModel.render();
        // 客户自定义字段渲染
    ...
    /**
     * @desc [改变客户角色时，进行相关权限控制，目前只有创始人能进行角色权限控制]
    ...
    var Status = (function () {
        ...
        function init(status) {
            ...
            if (!statusCheck(status)) {
                notice.warning('状态码不正确，客户状态初始化失败!');
                return;
            } else if (status === '9') {
                notice.warning('该客户已被删除!');
            }
        ...
        function regEvent() {
            ...
            $("#otherStatus").find('li').on('click', function (event) {
                ...
                if (!statusCheck(status)) {
                    notice.warning('状态码不正确，切换客户状态失败!');
...
//                        $li.find("div.phone-message").show();
    ...
    var initUserPermission = function (userType) {
        ...
        // 只有打开是客户时，才显示合并客户操作，才会显示归属客服组以及归属客服
        // 补录沟通小结、创建工单，任何类别角色都有
        // 删除客户按照权限来判断(不能操作自己信息，然后创始人>管理员>坐席>客户)
        // 修改密码能操作自己，并且权限低于自己的
        // 分配工单tab只有坐席和管理员有
