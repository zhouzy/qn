...
<div class="container-fluid">
    <ul class="breadcrumbs">
        <li>
            <a target="_self" href="<%=request.getContextPath()%>/userField/userfields">客户自定义字段</a>
        </li>
        <li>添加:[<span id="span1"></span>]</li>
    </ul>
    <div class="panel">
            ...
            <div class="kf5-section">
                <div class="field t-col">
                    <div class="t-col-content">
                        ...
                        <p class="hint">在客户信息页上显示的标题</p>
                        <h5><label><input name="checkbox" id="UserField_agent_required" maxlength="255" type="checkbox"> 是否必填</label></h5>
                        <h5> 描述（可选）</h5>
...
<script type="text/javascript">
    ...
    function addItem(){
        var spanHtml="";
        i=i+1;
        spanHtml+='<li id="index'+i+'">' +
            '<input id="items_'+i+'" name="" value="" placeholder="请输入选项内容" type="text">' +
            '<a target="_self" href="javascript:void(0);" class="remove" onclick="delItem('+i+');"></a>' +
            '</li>';
    ...
    function addType(){
        ...
        //选择正则表达式
        if("${type}"=="customized"){
            var candidateValue = $("#Customized_items").val();
            //绕过后台解码，这里编码两次
            candidateValue = encodeURIComponent(candidateValue);
            candidateValue = encodeURIComponent(candidateValue);
        ...
        if("${type}"=="phoneNum"){
            ...
            checklist[0] = "^\\d{7,8}$|^\\d{11,13}$";
            var _r =  encodeURIComponent(encodeURIComponent(checklist[0]));
            $("#candidateValue").val(JSON.stringify({
                "0" : _r
            }));
        }
        ...
        $.post("<%=request.getContextPath()%>/userField/addDefinedField", $("#fieldForm").serialize(),function (data) {
            if (data.success) {
                var title=data.rows;
